"""Train an adversarial self-play RL policy against a frozen teacher checkpoint."""
...
from pokestrategist.training.adversarial_rl import AdversarialRLConfig, train_adversarial_policy


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train an adversarial self-play RL refinement from a v1 checkpoint."
    )
    ...
    parser.add_argument("--reference-checkpoint", default=None,
                        help="Frozen teacher checkpoint. Defaults to --checkpoint.")
    ...
    parser.add_argument("--min-learning-rate-ratio", type=float, default=0.4)
    ...
    parser.add_argument("--convert-score-weight", type=float, default=0.5)
    parser.add_argument("--safe-score-weight", type=float, default=0.15)
    parser.add_argument("--tempo-score-weight", type=float, default=0.15)
    parser.add_argument("--resource-score-weight", type=float, default=0.2)
    parser.add_argument("--switch-sample-weight", type=float, default=1.0)
...
def main() -> None:
    args = _parse_args()
    metrics = train_adversarial_policy(
        args.data,
        args.output_dir,
        checkpoint_path=args.checkpoint,
        reference_checkpoint_path=args.reference_checkpoint,
        rl_config=AdversarialRLConfig(
            epochs=args.epochs,
            ...
            convert_score_weight=args.convert_score_weight,
            safe_score_weight=args.safe_score_weight,
            tempo_score_weight=args.tempo_score_weight,
            resource_score_weight=args.resource_score_weight,
