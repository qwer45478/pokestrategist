...
from pokestrategist.cli.train_adversarial_rl import main as adv_main
from pokestrategist.models import PokeStrategistDecisionModel

from tests.v1_fixtures import write_dataset


def test_train_adversarial_rl_cli_smoke(tmp_path, monkeypatch):
    ...
    output_dir = tmp_path / "adv_rl_run"

    monkeypatch.setattr(
        sys,
        "argv",
        [
            "train_adversarial_rl",
            "--data", str(dataset_path),
            "--checkpoint", str(checkpoint_path),
            "--reference-checkpoint", str(checkpoint_path),
    ...
    assert "adv_policy_advantage" in best_metrics
    assert "adv_student_expected_value" in best_metrics

    # Verify the checkpoint is loadable by evaluate_v1
    monkeypatch.setattr(
        sys,
        "argv",
        [
